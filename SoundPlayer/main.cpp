...
TCHAR name[100];		// フォルダ名
TCHAR dir[100];		// フォルダのディレクトリ名
...
// 関数
// ダイアログ表示
void DaialogView(void)
{
	// ディレクトリ選択項目の表示
	binfo.hwndOwner = NULL;
	binfo.pidlRoot = NULL;
	binfo.pszDisplayName = name;
	binfo.lpszTitle = L"BGMを選択してください";
...
void SetBGM(void)
{
	...
	{
		MessageBox(NULL, L"ディレクトリ選択されませんでした", L"キャンセル", MB_OK);
	}
	else
	{
		// 与えられたディレクトリを格納する
	...
	// サウンドファイルの読み込み
	SoftSoundHandle = LoadSoftSound(dir);
	...
	if (SoftSoundHandle == -1)
	{
		...
		MessageBox(NULL, L"これはオーディオファイルではありません", L"エラー", MB_OK);
	}
	else
	{
		// サウンドファイルの長さを取得
...
void PlayAndStop(void)
{
	...
	DrawFormatString(200, 30, 0xffffff, L"Zキー:一時停止");
	DrawFormatString(400, 30, 0xffffff, L"Xキー:先頭から再生");
}
...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nCmdShow)
{
	...
	while (ProcessMessage() == 0)
	{
		...
		DrawFormatString(120, 0, 0xffffff, L"現在再生中のBGM: ");
		if (SoftSoundHandle == -1)
		{
			DrawFormatString(280, 0, 0xffffff, L"BGMが選択されていません", loopFlag);
		}
		else
		{
			DrawFormatString(280, 0, 0xffffff, L"%s", name);
		}
		// 裏画面の内容を表画面に反映
		ScreenFlip();
	}
