...
PitchDown::PitchDown(double rate)
{
	rate_ = rate;	// 音の高さの変更
...
void PitchDown::GenelatePitchDownWaveFile(const wchar_t* fileName, const wchar_t* afterFileName)
{
	// Wavを読み込む
	lpWave.WaveRead(*pcm0_, fileName);

	// 元のPCMと変換後のPCMの値をセットする
	pcmSet_->PCMSetPitchDown(*pcm1_, *pcm0_, rate_);
	...
	while (channelL_->offset0 + channelL_->pmax * 2 < pcm0_->length)
	{
		...
		{
			// 単調減少の重み付け
			pcm1_->sL[channelL_->offset1 + n] = pcm0_->sL[channelL_->offset0 + n] * (channelL_->p - n) / channelL_->p;
			// 単調増加の重み付け
			pcm1_->sL[channelL_->offset1 + n] += pcm0_->sL[channelL_->offset0 + channelL_->p + n] * n / channelL_->p;
		}

		channelL_->q = static_cast<int>(channelL_->p / (rate_ - 1.0) + 0.5);
		for (int n = channelL_->p; n < channelL_->q; n++)
		{
			...
			pcm1_->sL[channelL_->offset1 + n] = pcm0_->sL[channelL_->offset0 + channelL_->p + n];
		}

		// オフセットの更新
		// 更新したら次のサンプルへ
		channelL_->offset0 += channelL_->p + channelL_->q;
	...
	while (channelR_->offset0 + channelR_->pmax * 2 < pcm0_->length)
	{
		...
		{
			// 単調減少の重み付け
			pcm1_->sR[channelR_->offset1 + n] = pcm0_->sR[channelR_->offset0 + n] * (channelR_->p - n) / channelR_->p;
			// 単調増加の重み付け
			pcm1_->sR[channelR_->offset1 + n] += pcm0_->sR[channelR_->offset0 + channelR_->p + n] * n / channelR_->p;
		}

		// 基準時刻の更新
		channelR_->q = static_cast<int>(channelR_->p / (rate_ - 1.0) + 0.5);
		for (int n = channelR_->p; n < channelR_->q; n++)
		{
			...
			// サンプルのコピー
			pcm1_->sR[channelR_->offset1 + n] = pcm0_->sR[channelR_->offset0 + channelR_->p + n];
		}

		// offset0の更新
		channelR_->offset0 += channelR_->p + channelR_->q;
		// offset1の更新
